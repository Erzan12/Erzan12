...
CATEGORY_KEYWORDS = {
    ...
    'tools' : ['api', 'restfule', 'task-manager', 'expense', 'post-management', 'beginner']
}

def get_category(repo_name, topics, description):
    """Determin category based on repo name, topics and description"""
...
def fetch_github_repos():
    ...
    try: 
        ...
        # Filter out fork repos and sort by updated date
...
def generate_project_html(repos):
    ...
    for repo in repos:
        # Fetch langues for the repo
        languages = {}
        try: 
            lang_response = request.get(repo['languages_url'])
        ...
        # Determin link (homepage or repo URL)
        ...
        html = f'''
                <div class="project-item link-card bg-slate-800/30 border border-slate-700 hover:border-accent rounded-xl p-4" data-category="{category}">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <a href="{link_url}" class="text-[10px] px-2 py-1 rounded bg-slate-700 hover:bg-white hover:text-maritime transition-all font-bold">{link_text}</a>
                    </div>
...
def update_html_file(project_html):
    ...
    try:
        ...
        end_marker = '</div>\n        </section>\n\n        <section class="w-full text-left pt-6 border-t border-slate-800">\n            <h2 class="text-lg font-bold text-white mb-4 flex items-center gap-2">\n                <span class="text-accent"><i class="fas fa-toolbox"></i></span> Skill Stack'
